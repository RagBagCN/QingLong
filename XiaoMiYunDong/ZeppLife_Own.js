 ...
 * 日期：6-6
 * 两种方式
 * ① 配置文件添加：export xmbs='小米账号1&密码&步数(换行)小米账号2&密码&步数'  多个账号用换行分割
 * ② 环境变量格式：名称“xmbs”  值“小米账号1&密码&步数(换行)小米账号2&密码&步数” 多个账号用换行分割
 * 定时一天一次
 * 更新成换行登录
 */

 const $ = new Env('ZeppLife 步数修改');
 ...
 !(async () => {
     ...
     if (!(await Envs()))
         ...
         for (let index = 0; index < xmbsArr.length; index++) {
             ...
             /** bs[] 账号&密码&步数   步数=设定步数+随机步数(0,2000) */
             let num = index + 1
             bs = xmbsArr[index].split('&');
             bs[2] =+ bs[2];
             bs[2] += randomInt(0,5000);
